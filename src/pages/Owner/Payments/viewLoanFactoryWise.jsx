    ...
    <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-auto p-8">
      <div className="flex items-center justify-between mb-6 border-b pb-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-base">Visual breakdown of total loan amount given to suppliers, by factory.</p>
        </div>
        <button
          onClick={() => navigate(-1)}
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-2xl font-bold text-blue-700">LKR {total.toLocaleString()}</span>
        </div>
      </div>
      <div className="mb-8">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <Legend />
            <Bar dataKey="amount" fill="#2563eb" name="Loan Amount (LKR)" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map(fp => (
              <tr key={fp.factory}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-blue-700 font-bold">LKR {fp.amount.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
