...
export default function ViewPaymentFactoryWise() {
  ...
  const title = location.state?.title || 'Factory-wise Payment';
    ...
    <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-auto p-8">
      <div className="flex items-center justify-between mb-6 border-b pb-4">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-base">Visual breakdown of total tea leaves payment, by factory.</p>
        </div>
        <button
          onClick={() => navigate(-1)}
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3">
          <span className="text-lg font-semibold text-gray-700">Total Payment:</span>
      ...
      <div className="mb-8">
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <Legend />
            <Bar dataKey="amount" fill="#16a34a" name="Tea Payment (LKR)" radius={[8, 8, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map(fp => (
              <tr key={fp.factory}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-green-700 font-bold">LKR {fp.amount.toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
